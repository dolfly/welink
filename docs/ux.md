...
在输入框里按下不会拦截，专心打字时不用担心误触。绑定只覆盖前 9 个 Tab；定时任务、AI 画廊、设置等 Tab 通过侧边栏点选进入，没有对应的 ⌘ 数字键。
...
群聊画像「成员榜」里点任意成员（不限好友），弹出 TA 的群内专属画像：群内排名 / 消息占比 / 活跃天数 / 深夜发言比例、24h 时段分布、周分布、近一年月度趋势、消息类型分布、TA 的高频词、最近发言样本。是好友的还会出现「完整分析」入口，一键跳到该好友的私聊详情。

数据走 `GET /api/groups/member-detail?username=<群wxid>&member=<成员wxid>`（两个参数都必填）。

## 聊天回放（私聊 / 群聊）

私聊详情和群聊详情都有回放播放器，按真实时间间隔回放消息：

- **倍速**：实时 / 2× / 5× / 10× / 50× / 100× / 200× 共 7 档
- **全屏回放**：进入沉浸式全屏，`Esc` 退出
- **智能节奏**：冷场（间隔 >5 分钟）直接跳过；发言密集时按内容长度放慢到刚好能读完
- **快进步长**：可切 3 / 5 / 10 条
- **快捷键**：`空格` 播放 / 暂停，`→` 快进

群聊回放额外按最近 N 条 / 日期范围加载，连续同发言人合并头像 + 日期分割线。

## 定时任务

侧边栏「定时任务」Tab（有新结果时显示未读红点角标）。写一条指令 + 选目标会话 + 选周期，系统就定时跑 LLM，把结果送进应用内收件箱（markdown）：

- **任务类型**：定时总结 / 待办挖掘 / 关键词盯梢 / 情绪变化 / 自定义 prompt
- **分析范围**：某个私聊 / 某个群 / 全部私聊 / 全部群聊
- **周期**：每天固定时间 / 每周几固定时间 / 每 N 小时间隔

纯本地 App 调度，**只在打开时执行**；关闭期间错过的下次打开补跑一次（不堆积，避免一开就刷一堆）。每个任务能「立即执行一次」，也能查运行历史；收件箱可一键全部标记已读。
